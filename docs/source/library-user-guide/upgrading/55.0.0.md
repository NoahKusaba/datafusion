...
### `LogicalExtensionCodec` can tell a write target from a scan

The target of a DML statement is serialized as a synthetic
`LogicalPlan::TableScan`, so on decode a `LogicalExtensionCodec` had no way to
tell the target of an `INSERT` from a table that is genuinely being read. Codecs
that build a cheap read-only provider — one pinned to a snapshot, say — produced
a provider that could not be written to, with no error to point at.

`LogicalExtensionCodec` gains a defaulted method that carries this context:
...
DataFusion now calls this method rather than `try_decode_table_provider`. **No
existing implementation breaks**: the default delegates to
`try_decode_table_provider`, which is unchanged and still required.

Implement the new method only if the distinction matters to you:
...
fn try_decode_table_provider_with_args(
    ...
    match args.usage() {
        TableProviderUsage::Scan => {
            // read-only, pinned to whatever snapshot the bytes encode
            Ok(Arc::new(MyReadOnlyProvider::decode(args.buf(), args.schema())?))
        }
        TableProviderUsage::WriteTarget => {
            // resolved through the catalog so `insert_into` works
            let table = my_catalog(ctx)?.load(args.table_ref())?;
            Ok(Arc::new(MyWritableProvider::new(table)))
        }
        // `TableProviderUsage` is `non_exhaustive`
        usage => not_impl_err!("unsupported table provider usage {usage:?}"),
    }
}
```

Two things to know about `TableProviderUsage`:

- It is `non_exhaustive`, so a `match` on it needs a catch-all arm. Further
  usages may be described in a future release.
- `WriteTarget` covers every `WriteOp`, not just `INSERT`. `UPDATE`, `DELETE`
  and `TRUNCATE` targets are all reported this way. Note also that `UPDATE` and
  `DELETE` resolve their table once and use it both as the scanned input and as
  the DML target, so a codec sees the same `table_ref` twice for such a plan:
  once as `Scan`, once as `WriteTarget`.

### `FFI_LogicalExtensionCodec` gained a field

`datafusion_ffi::proto::logical_extension_codec::FFI_LogicalExtensionCodec` is
`repr(C)`, and its `try_decode_table_provider` function pointer takes an extra
`u8` carrying the `TableProviderUsage` discriminant. This changes the struct
layout, so providers and consumers must be built against the same major
DataFusion version — which the existing `version()` check already enforces.

The usage crosses the boundary as a `u8` rather than as the enum, so a value
from a peer built against a newer DataFusion surfaces as a decode error rather
than as an invalid discriminant. Convert with `u8::from` and
`TableProviderUsage::try_from` rather than transmuting.
